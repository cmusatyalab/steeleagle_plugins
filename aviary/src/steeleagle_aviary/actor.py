...
from datatypes import GeodeticPoint
...
class Actor:
    def __init__(self,
        ...
        if 'waypoints' in kwargs:
            ...
            for w in kwargs['waypoints']:
                ...
                self.waypoints.put(self.convert_to_sim(gp))
    ...
    def set_next_waypoint(self):
        """Attempts to set the actor target position to the next waypoint
        by consuming a waypoint from the front of the waypoint queue. For cycled
        traces intended to run indefinitely use cycle_next_waypoint.
        If no waypoint queue exists for the actor, the target position is set
        ...
        if self.waypoints is not None and not self.waypoints.empty():
            ...
            print(f'{self.waypoints.qsize()} waypoints remaining')

    def cycle_next_waypoint(self):
        """Attempts to set the actor target position to the next waypoint
        after placing the current waypoint back on the actor's waypoint queue.
        If no waypoint queue exists, the target position is set to the current
        position. For traces intended to run the path a single time, use
        set_next_waypoint instead.
        """
        if self.waypoints is not None and not self.waypoints.empty():
            self.waypoints.put(self.position_target)
            self.set_position_target(self.waypoints.get())
